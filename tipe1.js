(() => {
  ...
  function renderPreview() {
    ...
    ui.fileName.textContent = `${String(state.previewIndex + 1).padStart(3, "0")}-tipe-1.png`;
  }
  ...
  async function generateAll() {
    ...
    try {
      ...
      zip.file("INFO.txt", [
        "Template Instagram Tipe 1 — Bagian 4",
        `Jumlah gambar: ${total}`,
        "Ukuran: 2160 x 2160 px",
        `Latar: ${BACKGROUND}`,
        "Tanda: wordivee",
        "Posisi quotes: rata kiri, blok teks di tengah vertikal",
        "",
      ...
      downloadBlob(output, `template-tipe-1-${total}-${date}.zip`);
      setProgress(100, total, total, "Selesai!");
      await delay(350);
      closeProgress();
      showToast(`${total} gambar Tipe 1 berhasil diunduh.`);
    } catch (error) {
      console.error(error);
      closeProgress();
  ...
  function setProgress(percent, current, total, title) {
    ...
    ui.progressDetail.textContent = safe < 90 ? "Merender PNG 2160 × 2160 px." : "Menyiapkan unduhan.";
  }
